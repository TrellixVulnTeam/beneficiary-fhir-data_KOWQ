...
# Beneficiaries
#
# Beneficiaries, which captures one row for each beneficiary queried
...
resource "aws_glue_workflow" "glue-workflow-populate-beneficiaries" {
  name = "${local.full_name}-populate-beneficiaries-workflow"
  max_concurrent_runs = "1"
}
...
resource "aws_glue_trigger" "glue-crawler-api-requests-crawler" {
  ...
  workflow_name = aws_glue_workflow.glue-workflow-populate-beneficiaries.name
  type          = "ON_DEMAND"
