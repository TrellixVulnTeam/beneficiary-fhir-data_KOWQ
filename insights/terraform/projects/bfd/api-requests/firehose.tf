...
resource "aws_kinesis_firehose_delivery_stream" "firehose-ingester" {
  ...
  extended_s3_configuration {
    ...
    buffer_interval     = 60
    buffer_size         = 128
    error_output_prefix = "temp-database/${module.database.name}/${module.glue-table-api-history.name}_errors/!{firehose:error-output-type}/year=!{timestamp:yyyy}/month=!{timestamp:MM}/day=!{timestamp:dd}/"
    kms_key_arn         = data.aws_kms_key.kms_key.arn
    prefix              = "temp-database/${module.database.name}/${module.glue-table-api-history.name}/firehose:year=!{timestamp:yyyy};month=!{timestamp:MM}/day:!{timestamp:dd}/"
